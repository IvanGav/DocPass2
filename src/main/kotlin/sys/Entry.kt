...
class Entry private constructor(
    ...
    companion object {
         ...
         */
        fun empty(named: Boolean = true) = Entry(if(named) EntryTemplate.Named.map else EntryTemplate.None.map)

        /**
         * Create an empty file with empty ("") fields from a specified template (see ./FileTemplate.kt).
         */
        fun fromTemplate(template: EntryTemplate) = Entry(template.map)
    }
}